...
#        CineBaseSolver.__init__(CsTransform, data, mu, LMBD, gamma, nInner, nBreg)
    def data2rho(self, dd_raw,dim_x,dim_y,dim_z, dim_t,n_coil):
        '''
         dd_raw data in kx-ky-t space
         dd_raw, shape (512(kx), 1056(ky-kz-t), 4coil)
         
        '''
        if dd_raw.ndim != 3:
            print(' dimension is wrong! ')
        
        dd_raw = numpy.transpose(dd_raw,(1,2,0)) # kyt,coil,kx put the x-axis to the tail
        
        shape_dd_raw = numpy.shape(dd_raw) # shape_dd_raw[0]: ky+kz+t, [1] coil, [2] kx         
        dd_raw = numpy.reshape(dd_raw,
                               (shape_dd_raw[0],)+ (shape_dd_raw[1]*shape_dd_raw[2],),
                                order='F') #[0] ky+kz+t  [1] coil+kx
    
        output_data = self.backward(dd_raw) # (dim_y, dim_z, dim_t, n_coil*dim_x)
        print('in data2rho,')
    
        output_data = numpy.reshape(output_data, (dim_y,dim_z, dim_t,n_coil,dim_x),order='F')
    
        output_data = numpy.transpose(output_data, (4,0,1,2,3)) #(,dim_x, dim_y,dim_z, dim_t,n_coil)
...
#         CsTransform = self.CsTransform
        ...
        dd_raw = numpy.transpose(dd_raw,(1,2,3,4, 0)) # put the x-axis to the tail
        
        shape_dd_raw = numpy.shape(dd_raw)
        
        dd_raw = numpy.reshape(dd_raw,
                  (shape_dd_raw[0],)+ (shape_dd_raw[1],)+(shape_dd_raw[2],)+(shape_dd_raw[3]*shape_dd_raw[4],),
                        order='F')
...
#        output_data = CsTransform.backward(output_data)
        ...
        output_data = numpy.transpose(output_data, (4,0,1,2,3)) 
    ...
    def fun1(self,cineObj):
        ...
        '''
        output=self.data2rho( cineObj.f , cineObj.dim_x , self.st['Nd'][0],self.st['Nd'][1],self.st['Nd'][2],cineObj.ncoils)
...
#         m = scipy.fftpack.ifftn(m,axes=(0,))
    ...
    def fun3(self,m):
        '''
        1)shift-f axis 3
        2)fft: along f axis 3
        3)(No shift)-t axis 3
        '''
        m = scipy.fftpack.fftshift(m, axes=(3,))
        m= pyfftw.interfaces.scipy_fftpack.fftn(m, axes=(3,),threads=2, overwrite_x=True)
...
#         m = scipy.fftpack.fftn(m,axes=(2,)) 
    ...
    def fun4(self,m):
        ...
        3)shift-f
        '''
        m= pyfftw.interfaces.scipy_fftpack.ifftn(m, axes=(3,),threads=2, overwrite_x=True)
#         m = scipy.fftpack.ifftn(m,axes=(2,)) 
        
        m = scipy.fftpack.ifftshift(m, axes=(3,)) 
...
#             output_q = self.fast_fun4_FhF_fun3(output_q, cineObj)
    ...
    def fun5(self,m):
        ...
        1)shift-f:
        2)shift-x-y-z:
        2)fft2 along x-y-z, no shift -> kx ky f
        
        for 3D laplacian operator
        '''
        m = scipy.fftpack.ifftshift(m, axes=(0,1,2,3)) # shift along x,y,z,f
        m= pyfftw.interfaces.scipy_fftpack.fftn(m, axes=(0,1,2),threads=2, overwrite_x=True)
                                                                # fft along x y z
...
#         m = scipy.fftpack.ifftn(m,axes =  (0,1,)) 
        m= pyfftw.interfaces.scipy_fftpack.ifftn(m, axes=(0,1,2),threads=2, overwrite_x=True)              
        m = scipy.fftpack.fftshift(m, axes=(0,1,2,3))    
...
#        lapla_Wq=scipy.fftpack.ifftshift(q,axes=(2,))
    ...
    def inverse(self, cineObj, mu, LMBD, gamma, nInner, nBreg): # main function of solver
        ...
        self.nInner= nInner
        self.nBreg= nBreg
        u0=numpy.empty((cineObj.dim_x, self.st['Nd'][0], self.st['Nd'][1],self.st['Nd'][2], 4))
        
        tse = cineObj.tse
        
        tse = CsTransform.pynufft.appendmat(tse,u0.shape[3])# append temporal dimension
        tse = numpy.transpose(tse,(0,1,2,4,3)) # x,y,z,t, coil
...
#===============================================================================
        ...
        self.st = self._make_sense(cineObj.tse) # setting up sense map in st['sensemap']
        
        self.st['sensemap'] = CsTransform.pynufft.appendmat(self.st['sensemap'],u0.shape[3])
        self.st['sensemap'] = numpy.transpose(self.st['sensemap'],(0,1,2,4,3))
...
#         # related to constraint
#===============================================================================
        uker2 = numpy.zeros((cineObj.dim_x,)+self.st['Nd'][0:3],dtype=numpy.complex64)
#         rows_kd = self.st['Nd'][0] # ky-axis
        #cols_kd = self.st['Kd'][1] # t-axis
#        uker[0,0] = 1.0
        uker2[0, 0, 0,0] = -2.0
        ...
        uker2 = scipy.fftpack.fftn(uker2,axes=(0,1,2,3)) # 256x256x16
        return uker2       
...
#        tse=numpy.abs(numpy.mean(self.st['sensemap'],-1))
        ...
        tse=CsTransform.pynufft.appendmat(tse,self.st['Nd'][2]) # append coil dimension
        #tse=Normalize(tse)
        tse=numpy.transpose(tse,(0,1,2,4,3)) # put coil to the tail
        self.ttse=CsTransform.pynufft.Normalize(tse)
        
        self.tse0 = CsTransform.pynufft.CombineMulti(tse, -1)
        
        self.filter= numpy.ones(tse.shape)
        dpss = numpy.kaiser(tse.shape[1], 1.0)*10.0 # frequency filtering ky
        for ppp in range(0,tse.shape[1]):
            self.filter[:,ppp,:,:,:]=self.filter[:,ppp,:,:,:]*dpss[ppp]
        dpss2 = numpy.kaiser(tse.shape[2], 1.0)*10.0# filtering kz
        for ppp in range(0,tse.shape[2]):
            self.filter[:,:,ppp,:,:]=self.filter[:,:,ppp,:,:]*dpss2[ppp]        
...
#                         ) # doing spatial transform
        ...
        pdf = cineObj.pdf
        pdf = CsTransform.pynufft.appendmat(pdf,self.st['Nd'][2]) # append t dimension
        pdf = numpy.transpose(pdf,(0,1,2, 4,3)) # swap coil and t dimension, put coil to the tail
...
#            matplotlib.pyplot.show()
        ...
        uker = CsTransform.pynufft.appendmat(uker,u0.shape[4])
...
#                        )       
        ...
        for jj in range(0,3):
            m = self.cg_step(rhs,m,uker,3,cineObj)
...
#        return u       
    ...
    def external_update(self,u, f, uf, f0, u0): # overload the update function
        ...
        CsTransform.pynufft.checkmax(self.st['sensemap'])
...
#        fact=1.0/(1.0+numpy.exp(-(fact-0.5)*self.thresh_scale))
        ...
        CsTransform.pynufft.checkmax(tmpuf)
        CsTransform.pynufft.checkmax(u0)
        CsTransform.pynufft.checkmax(uf)
...
#                        )       
        ...
        for jj in range(0,2):
            m = self.cg_step(rhs,m,uker,2,cineObj)
...
#        uker[0,0] = 1.0
        rate = 40.0
        uker2[0,0,0,0] = -2.0 - 4.0/rate
        uker2[0,0,0,1] =1.0
        uker2[0,0,0,-1]=1.0
        uker2[0,0,-1,0]=1.0/rate
        uker2[0,0,-1,0]=1.0/rate
        uker2[0,1,0,0] =1.0/rate
        uker2[0,-1,0,0]=1.0/rate
        uker2[1,0,0,0] =1.0/rate
        uker2[-1,0,0,0]=1.0/rate        
        uker2 = scipy.fftpack.fftn(uker2,axes=(0,1,2,3)) # 256x256x16
        return uker2
    def constraint(self,xx,bb):
        ...
        '''
        cons = CsTransform.pynufft.TVconstraint(xx[0:3],bb[0:3]) * self.LMBD/40.0
        #cons =  CsTransform.pynufft.TVconstraint(xx[0:2],bb[0:2]) * self.LMBD/100.0
        #cons = cons + CsTransform.pynufft.TVconstraint(xx[2:3],bb[2:3]) * self.LMBD
        cons = cons + scipy.fftpack.ifftn(xx[4]-bb[4],axes = (3,))* self.gamma
        cons = cons + (xx[5]-bb[5])* self.gamma
        #cons = cons + xx[2]-bb[2]
...
#        print('inside_update_d fre grad ushape',freq_gradient(u).shape)
        ...
        for jj in range(0,len(dd)) :
            if jj < 4: # derivative y 
                #tmp_d =get_Diff(u,jj)
                out_dd = out_dd  + (CsTransform.pynufft.get_Diff(u,jj),)
            elif jj == 4: # rho
                tmpu = numpy.copy(u)
                tmpu = scipy.fftpack.fftn(tmpu,axes = (3,))
#                 tmpu[:,:,0,:] = tmpu[:,:,0,:]*0.0
                out_dd = out_dd + (tmpu,)
                
            elif jj == 5:
                average_u = numpy.sum(u,3)
                tmpu= numpy.copy(u)
...
#        output_xx = output_xx + (tmp_xx[2],)
        ...
        output_xx= CsTransform.pynufft.shrink( dd[0:3], bb[0:3], thrsld)# 3D thresholding
        output_xx=output_xx + CsTransform.pynufft.shrink( dd[3:4], bb[3:4], thrsld)# 3D thresholding 
        output_xx =output_xx + CsTransform.pynufft.shrink( dd[4:5], bb[4:5], thrsld)
        output_xx =output_xx + CsTransform.pynufft.shrink( dd[5:6], bb[5:6], thrsld)
