...
class atomic_computation2:
    def __init__(self,Cine2DSolver, cineObj ,tmp_file, tmp_shape, tmp_dtype, 
                  out_file, out_shape, out_dtype, dim_1,dim_2):
...
#         del ff
    ...
    def run(self,  zz, mu, LMBD, gamma, nInner, nBreg, ncpu):
        self.create(zz, ncpu)
        result = self.Cine2DSolver.pseudoinverse(self.cineObj, mu, LMBD, gamma, nInner, nBreg)
        
        outP= numpy.memmap(self.out_file, mode ='readwrite', shape = self.out_shape, dtype = self.out_dtype)
        
        if zz < ncpu -1:
#             new_x = my_atomic_calc[zz].cineObj.dim_x# numpy.ceil((1.0)*dim_x/ncpu).astype(int)
             
            outP[self.dim_1*zz:  self.dim_1*(zz+1),...] = result[:,...]
             
        else:
#             new_x = my_atomic_calc[zz].cineObj.dim_x #dim_x - (numpy.ceil((1.0)*dim_x/ncpu))*(ncpu - 1)
             
            outP[- self.dim_2:,...]= result[:,...]
        outP.flush()
        del outP
...
def wrapper_atomic_computation(my_atomic_calc, zz,  mu, LMBD, gamma, nInner, nBreg, ncpu):
    cProfile.runctx('wrapper_atomic_computation_kernel(my_atomic_calc, zz,  mu, LMBD, gamma, nInner, nBreg, ncpu)',
                    globals(), locals(), 'profile-%d.out' %zz)
    
def wrapper_atomic_computation_kernel(my_atomic_calc, zz,  mu, LMBD, gamma, nInner, nBreg, ncpu):
    pid= os.getpid()
    pinned_core = zz  % multiprocessing.cpu_count()
    os.system("taskset -p -c %d %d" % ( pinned_core , pid))
    my_atomic_calc.run( zz, mu, LMBD, gamma, nInner, nBreg, ncpu)
...
#     u_in_xyf  = numpy.empty((xres,Nd[0],Nd[1],ncoil),dtype =tmp_dtype)
    out_file = str("/tmp/tmpOut.dat")
    out_shape = (xres,Nd[0],Nd[1],ncoil)
    out_dtype = 'complex64'#str(  tmp_dtype)
    print(tmp_dtype)
    print(tmp_file)
    print(tmp_shape)
#     with open(tmp_file, 'write') as ff:
    mp = numpy.memmap(out_file , mode='w+', shape=out_shape, dtype = out_dtype)
...
#             tmpObj.f = F[ -dim_2:]
        ...
        my_atomic_calc.append( atomic_computation2(Cine2DSolver, tmpObj , tmp_file, tmp_shape, tmp_dtype, 
                                                   out_file, out_shape, out_dtype, 
                                                   dim_1,dim_2) )
...
#                          (my_atomic_calc[zz], zz,  mu, LMBD, gamma, nInner, nBreg, ncpu) ) )
        pool.apply_async(wrapper_atomic_computation, 
                         (my_atomic_calc[zz], zz,  mu, LMBD, gamma, nInner, nBreg, ncpu) ) 
#                          modules = ('numpy','pynufft','scipy'),
#                          globals = globals()))
...
#     job_server.destroy()
    ...
    mp = numpy.memmap(out_file , mode='readonly', shape=out_shape, dtype = out_dtype)
    u_in_xyf = numpy.array(mp)
    del mp    
    import platform
    ...
    #     print(mydir)
        ...
        for jj in xrange(0,num_of_series):
            for rfov_yres in (32, 256,):	# 64, 128,  ):#xrange(32, 256*2 -32,256-32):
                ...
                for nthread in ( 256, 128, 64, 32,16,8,4,2, 1 ): 	#1,2, 4, 8, 16, end_thread ):#, 64, 128, 256,512)+tuple(range(1,41))  :
    #                 jj = 2  
